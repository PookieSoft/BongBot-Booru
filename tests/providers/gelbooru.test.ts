...
describe('suggest', () => {
    const entry = { type: 'tag', label: 'furina (genshin impact)', value: 'furina_(genshin_impact)', post_count: '13542' };
    ...
    it('asks Gelbooru for tags matching the term', async () => {
        ...
        expect(params.get('term')).toBe('furina');
    ...
    it('returns nothing when Gelbooru answers with something other than a list', async () => {
        const get = vi.fn().mockResolvedValue({ '@attributes': { count: 0 } });
        await expect(new Gelbooru({ get }).suggest('furina')).resolves.toEqual([]);
    });
});
